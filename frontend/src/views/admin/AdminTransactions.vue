...
const showActions = computed(() => adminStore.isAdmin)

const fetch = async () => {
  loading.value = true
  const res = await adminGet(`/home/admin/transactions?page=${page.value}&pageSize=${pageSize.value}`)
...
const approve = async (id) => {
  ...
  const res = await adminPost('/home/admin/approve-transaction', { transactionId: id })
  actionLoading.value = ''
  if (res?.code === 0) {
...
const reject = async (id) => {
  ...
  actionLoading.value = id
  const res = await adminPost('/home/admin/reject-transaction', { transactionId: id })
  actionLoading.value = ''
  if (res?.code === 0) {
