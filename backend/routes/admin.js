...
router.put('/shops/:id/level', adminAuth, async (req, res) => {
  try {
    const { level } = req.body;
    if (!['bronze', 'silver', 'gold', 'platinum', ''].includes(level)) {
      return res.json(fail('Invalid level. Must be bronze, silver, gold, platinum, or empty.'));
    }
    const shop = await Shop.findByIdAndUpdate(req.params.id, { level }, { new: true });
    if (!shop) return res.json(fail('Shop not found'));
    res.json(success(shop, `Seller level set to ${level || 'none'}`));
  } catch (error) {
    res.json(fail(error.message));
  }
});

router.put('/shops/:id/credit-score', adminAuth, async (req, res) => {
  try {
    const { creditScore } = req.body;
    const score = Number(creditScore);
    if (isNaN(score) || score < 0 || score > 1000) {
      return res.json(fail('Credit score must be between 0 and 1000'));
    }
