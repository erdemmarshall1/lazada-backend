...
async function main() {
  ...
  const toUpload = all.filter(p => p.images?.[0]?.startsWith('/uploads/product_images/'));
  const missing = all.filter(p => p.images?.[0]?.startsWith('/uploads/') && !p.images[0].includes('product_images/'));
  console.log('product_images-path (uploadable): ' + toUpload.length);
  console.log('UUID-path (need alternative): ' + missing.length);
  ...
  for (let i = 0; i < toUpload.length; i++) {
    const p = toUpload[i];
    const imgUrl = IMG_SOURCE + p.images[0];
    ...
    if ((i + 1) % 20 === 0) console.log(`  ${i+1}/${toUpload.length}: ${ok} OK, ${fail} failed`);
  ...
  console.log(`Missing UUID-path products: ${missing.length}`);
}
